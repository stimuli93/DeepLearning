...
def softmax_loss_forward(x,y):
    """
    Inputs
    x = numpy array of shape (N,C) representing input layer
    y = numpy array of shape (N,) representing labels of each training
    example
...
def softmax_loss_backward(dout,cache):
    """
    Inputs
    dout - Gradient coming from 1 layer deeper
    cache - (x,y) used for backpropagation
